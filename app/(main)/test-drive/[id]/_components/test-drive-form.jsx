...
export function TestDriveForm({ car, testDriveInfo }) {
      ...
      try {
        ...
        for (let hour = openHour; hour < closeHour; hour++) {
          ...
          const isBooked = existingBookings.some((booking) => {
            ...
            const sameDate = format(bookingDate, "yyyy-MM-dd") === format(selectedDate, "yyyy-MM-dd");
          ...
          if (!isBooked) {
            slots.push({
              id: `${startTime}-${endTime}`,
              label: `${format(parseISO(`2000-01-01T${startTime}`), 'h:mm a')} - ${format(parseISO(`2000-01-01T${endTime}`), 'h:mm a')}`,
              startTime,
              endTime,
            });
          }
  ...
  const onSubmit = async (data) => {
    ...
    const [startTime, endTime] = selectedSlot.id.split('-').map(t => t.trim());
